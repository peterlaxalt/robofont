...
def _parse(text):
    ...
    for line in text.splitlines():
        ...
        result.append((minValue, maxValue, name))
    result = list(sorted(result))
    return result


_scripts = _parse(_scriptsText)
_blocks = _parse(_blocksText)

# functions


def script(value):
    scriptName = _search(value, _scripts)
...
def block(value):
    blockName = _search(value, _blocks)
    if blockName is None:
        blockName = "No_Block"
    return blockName


def _search(value, available):
    count = len(available)
    if count == 0:
        return None
    elif count == 1:
        test = available[0]
        lessRange = greaterRange = []
    else:
        half = int(count / 2)
        test = available[half]
        lessRange = available[:half]
        greaterRange = available[half:]
    minValue, maxValue, name = test
    if value < minValue:
        return _search(value, lessRange)
    elif value > maxValue:
        return _search(value, greaterRange)
    else:
        return name


def recursiveDecomposition(uniValue):
    ...
    chrs = [chr(int(i, 16)) for i in parts if i]
    letters = [ord(i) for i in chrs if unicodedata.category(i) in letterCategories]
    letterCount = len(letters)
    if letterCount != 1:
        return -1
    decomposedUniValue = letters[0]
    furtherDecomposedUniValue = recursiveDecomposition(decomposedUniValue)
    if furtherDecomposedUniValue != -1:
        furtherFurtherDecomposedUniValue = recursiveDecomposition(furtherDecomposedUniValue)
